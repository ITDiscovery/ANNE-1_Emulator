; --- ANNE-1 HARDWARE MAPPING ---
UART      EQU  $FF68      ; Status / Control
RECEV     EQU  $FF69      ; Receive Data
TRANS     EQU  $FF69      ; Transmit Data
USTAT     EQU  UART       ; Status
UCTRL     EQU  UART       ; Control
...
; --- OPCODES ---
SKP1      EQU  $21            ; OP CODE OF BRN
SKP2      EQU  $8C            ; OP CODE OF CMPX
SKP1LD    EQU  $86            ; OP CODE OF LDA
...
* --- RAM RESIDENT PLACEHOLDERS ---
GETNCH    RMB  20              ; Space for RAM routine
GETCCH    EQU  GETNCH+8       ; [FIX] Point GETCCH to the LDA [CHARAD] inside GETNCH
CHARAD    RMB  2              ; Input Pointer
          RMB  3              ; Space for JMP
...
LA077     CLR  ,-X            ; Pre-decrement by 1 (Simpler)
          CMPX #0             ; Compare X to 0 to set Flags
          BNE  LA077          ; Branch if X is not 0
...
LA093     STX  TOPRAM         ; SAVE ABSOLUTE TOP OF RAM
          ...
          LDB  #24            ; MOVE 24 BYTES
          JSR  LA59A          ; MOVE 24 BYTES FROM ROM TO RAM
...
LA3CD     PSHS CC             ; SAVE CARRY FLAG
          ...
          LDX  #LINBUF-1      ; RESET INPUT BUFFER POINTER
...
BROMHK    CMPA #'9+1          ; IS THIS CHARACTER > '9'?
          BHS  LAA28          ; BRANCH IF > 9 (Returns C=0)
          CMPA #SPACE         ; SPACE?
          BNE  LAA24          ; NO - GO CHECK DIGIT LOGIC
          JMP  GETNCH         ; IF SPACE, GET NEXT CHAR
...
TOK_INPUT EQU  $89
          ...
          FDB  RENUM          ; 9D
          FDB  EDIT           ; 9E
TOK_HIGH_EXEC EQU  $9E        ; Hardcoded to fix calculation error
...
LAF67     LDX  #0             ; Load 0 immediate. Preserves A. Ignores ZERO var.
          STX  <BINVAL        ; Store to BINVAL (Direct Page access)
          JSR  BROMHK         ; Check A: Set Carry if Numeric (0-9), Clear if not.
...
LB2C9     PULS B              ; GET TOKEN OFFSET
          LDX  COMVEC+9       ; GET SECONDARY FUNCTION JUMP TABLE ADDRESS
...
LIST      PSHS CC             ; SAVE ZERO FLAG ON STACK
          ...
          JSR  GETCCH         ; GET CURRENT CHARACTER FROM BASIC
          BEQ  LB784          ; Jump to LB784 (Set $FFFF default)
...
*
LB821     LDX  <CHARAD        ; [PATCH] Use Direct Page (2 bytes vs 3)
          LDU  #LINBUF        ; POINT U TO LINE INPUT BUFFER
LB829     CLR  <V43           ; [PATCH] Use Direct Page (2 bytes vs 3)
          CLR  <V44           ; [PATCH] Use Direct Page (2 bytes vs 3)
...
LB892     LEAX -1,X           ; MOVE INPUT POINTER BACK ONE
          ...
          LDU  #COMVEC-5     ; POINT U TO COMMAND INTERPRETATION
*                             ; TABLE FOR BASIC - 10
LB89B     CLR  V42            ; INITIALIZE V42 AS TOKEN COUNTER
LB89D     LEAU 5,U           ; MOVE TO NEXT COMMAND INTERPRETATION TABLE
...
LB8D4     LDU  #COMVEC_S-5    ; Start Pass 2 at Secondary Table
...
LBC2F     LDX  #V40           ; POINT X TO MANTISSA OF FPA3
          FCB  SKP2           ; SKIP TWO BYTES
LBC33     LDX  <VARDES        ; Force Direct Page (2 bytes) to fit inside SKP2
...
; --- PASTED DATA TABLES START HERE ---
* THESE BYTES ARE COPIED TO RAM AT STARTUP
LA10D     FCB  16             ; TAB FIELD WIDTH
          FCB  64             ; LAST TAB ZONE
          FCB  80             ; PRINTER WIDTH
          FCB  0              ; LINE PRINTER POSITION
          FDB  LB44A          ; EXEC COMMAND DEFAULT
* LINE INPUT ROUTINE RAM IMAGE
          INC  CHARAD+1
          BNE  LA123
          INC  CHARAD
LA123     LDA  [CHARAD]       ; Load Char (Sets Z if 0)
          BEQ  LRET         ; If 0, Exit (Preserve Z=1)
          JMP  BROMHK         ; Else, Go to ROM Hook
LRET      RTS                 ; Return
* VECTORS RAM IMAGE
          JMP  BIRQSV
          JMP  BFRQSV
...
COMVEC    FCB  50             ; 50 BASIC COMMANDS
          ...
          FCB  0              ; Terminator for Pass 1 (Commands)
COMVEC_S  FCB  29             ; Label for Pass 2 (Functions)
          FDB  LAB1A
